...
// 自动更新链路才不会断。
//
// 接法：electron-builder 26 用 -c.win.signtoolOptions.sign=build/evsign.cjs
// （本仓库 package.json 未声明 "type":"module"，是 CommonJS，用 .cjs 更明确）。
...
exports.default = async function sign(configuration) {
  ...
  // 白名单：只签 TaskList 自己的产物 —— 主程序 TaskList.exe / 安装器 TaskList-Setup-*.exe /
  // 卸载器 Uninstall TaskList.exe，它们文件名都含 "tasklist"。electron-builder 还会把一堆
  // 第三方捆绑 exe 送来签(node_modules 里的辅助 exe、resources/elevate.exe(NSIS 提权工具) 等)，
  // 这些不是我们的软件、不在 EV Sign 白名单 → 会返回"此文件未审核"报错拖垮整个打包。
  // 跳过它们是正确的：Windows SmartScreen 只校验下载到的安装器和主程序签名。
  const base = file.replace(/\\/g, "/").split("/").pop() || "";
  if (!/tasklist/i.test(base)) {
    console.log(`[evsign] 跳过非 TaskList 自有文件(不在签名白名单): ${file}`);
    return;
  }
